...
export class TileEntity extends GameEntity<ILevelProps> {
    ...
    onInit? = (self: IGameEntity<ILevelProps>) => {
        ...
        // First, initialize the entire logicalCollisionMap with 0s.
        // A value of 0 indicates a non-solid tile.
        // Updated initialization to handle the new object structure
        ...
        // Now, iterate through the indexed tiles to build the collision map.
        // This is a more direct approach since indexedTiles contains the pre-calculated coordinates.
        self.props.indexedTiles.forEach(tile => {
            const tileProps = getTileProperties(tile.type as keyof typeof TileDefinitions)!;
            
            // This `if` block handles all solid tiles EXCEPT tile 7.
            if (isSolidTile(tile.type) && !tileProps?.creator && tile.type != 7) { 
                ...
                // This `else` block now specifically handles tile 7.
                // We'll calculate the collision map for this tile here.
                if (tile.type === 7 && isSolidTile(tile.type)) {

                    console.log(getTilesByType(this.props.tileMap,7));                    
                    // Check for a specific collision box size, otherwise use the tile's visual dimensions
                    const collisionWidth = 112;
                    const collisionHeight = 224;
                    
                    // Calculate the logical grid coordinates for the top-left corner of the tile
                    const startCol = Math.floor(tile.x / self.props.tileWidth);
                    const startRow = Math.floor(tile.y / self.props.tileHeight);

                    // Use the collision dimensions to calculate how many base grid cells to fill
                    const cellsX = Math.floor(collisionWidth / self.props.tileWidth);
                    const cellsY = Math.floor(collisionHeight / self.props.tileHeight);

                    // Mark all occupied logical grid cells as solid
                    for (let x = 0; x < cellsX; x++) {
                        for (let y = 0; y < cellsY; y++) {
                            const logicalRow = startRow + y;
                            const logicalCol = startCol + x;
                            // Check bounds before marking to prevent errors
                            if (logicalRow < this.logicalCollisionMap.length && logicalCol < this.logicalCollisionMap[0].length) {
                                // Store the tile type and the top-left coordinates of the tile
                                this.logicalCollisionMap[logicalRow][logicalCol] = { type: tile.type, x: tile.x, y: tile.y, row: logicalRow, col: logicalCol };
                            }
